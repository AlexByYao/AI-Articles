...
root = r"E:\\code\\AI-Articles"
md = os.path.join(root, "interview", "cpp-qt", "2026-03-03", "questions.md")
...
cpp = [
    ...
    ("C++ 调试", "如何使用断言与日志定位问题", "断言用于捕获不变量，日志记录上下文；发布版可关闭断言，日志需分级。", 'easy'),
    ("C++ 构建", "预编译头的作用与风险", "加速编译但可能引入隐藏依赖；应控制头文件稳定性，避免滥用。", 'medium'),
]
...
assert len(extra) == 100, len(extra)
...
out = os.path.join(root, "docs", "public", "data", "2026-03-03.json")
with io.open(out, 'w', encoding='utf-8') as f:
    f.write(json.dumps({'questions': final}, ensure_ascii=False, indent=2))
